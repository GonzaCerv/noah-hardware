...
bool check_package_format() {
    ...
    if (buffer[USART_BUFFER_SIZE - 1] == STOP_PACKAGE) {
        return false;
    }
    return true;
}
...
bool send_off_command() {
    ...
    HAL_UART_Transmit_DMA(huartx, (uint8_t *)&response, sizeof(response));
        ...
        if (check_package_format()) {
            ...
            switch (buffer[1]) {
                case COMMAND_ECHO:
                    ...
                    HAL_UART_Transmit_DMA(huartx, (uint8_t *)&response, 4);
                    break;
                ...
                case COMMAND_MOVE:
                    ...
                    if (fabs(aux) > MOTOR_MAX_SPEED) aux = MOTOR_MAX_SPEED;
                ...
                case COMMAND_DETECT_IR_FRONT:
                    ...
                    HAL_UART_Transmit_DMA(huartx, (uint8_t *)&response, 3);
                    break;
