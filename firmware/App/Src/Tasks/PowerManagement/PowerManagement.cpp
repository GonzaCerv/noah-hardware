...
PowerManagement::PowerManagement(const Pin &led_1, const Pin &led_2, const Pin &en_power_3v3, const Pin &smps_pwr,
                                 const Pin &enable_pressed, const Pin &charger_connected, const Pin &standby_h_bridge) :
    led_1_ { led_1 },
    led_2_ { led_2 },
    en_power_3v3_ { en_power_3v3 },
    smps_pwr_ { smps_pwr },
    enable_pressed_ { enable_pressed },
    charger_connected_ { charger_connected },
    standby_h_bridge_ { standby_h_bridge } {
...
bool PowerManagement::isButtonPressed() {
  if (HAL_GPIO_ReadPin(enable_pressed_.port, enable_pressed_.pin) == GPIO_PIN_SET) {
    osDelay(30);
    if (HAL_GPIO_ReadPin(enable_pressed_.port, enable_pressed_.pin) == GPIO_PIN_SET) {
      return true;
    }
...
bool PowerManagement::isChargerConnected() {
  if (HAL_GPIO_ReadPin(charger_connected_.port, charger_connected_.pin) == GPIO_PIN_SET) {
    return true;
  }
  return false;
}
...
void PowerManagement::updateState() {
  if ((isButtonPressed()) && (!is_robot_on_)) {
    ...
    do {
      osDelay(10);
    } while (HAL_GPIO_ReadPin(enable_pressed_.port, enable_pressed_.pin) == GPIO_PIN_SET);
    // Turn on H bridge.
